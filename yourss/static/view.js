                    ...
                    if (channel.enabled) {
                        ...
                        $.get("/api/channel/" + channel.channel_id + "/fetch", function (feed) {
                            $.each(feed.videos, function (i, video) {
                                $("#myVideos").append(`
                                    <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 mb-3 filterable" data-channel="${channel.channel_id}" data-timestamp="${video.updated}" data-label="${channel.label}">
                                        <div class="card h-100" data-thumbnail="${video.thumbnail_url}" data-video="${video.id}">
                                            ...
                                            <div class="card-body d-flex flex-column">
                                                ...
                                                <p class="card-text" style="cursor: pointer;" onclick="window.open('${video.link}')">${video.title}</p>
                                                <p class="card-text mt-auto"><small class="text-muted">${moment(video.updated).fromNow()}</small></p>
                                            </div>
                                        </div>
                                    </div>
                                `)
                            })
                            sort_list("#myVideos", function (x) { return new Date($(x).data("timestamp")).getTime() * -1 })
                        })
                    }
                })
